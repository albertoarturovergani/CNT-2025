# coding: utf-8
"""
Example from https://neuralensemble.org/docs/PyNN/examples/VAbenchmarks.html
...
if options.benchmark == "COBA":
    rec_vars = ['v', 'gsyn_exc', 'gsyn_inh']
else:
    rec_vars = ['v']

if options.use_views or options.use_assembly:
    all_cells.record('spikes')
    exc_cells[[0, 1]].record(rec_vars) # is recording two cells, for getting all the pop cells use  exc_cells.record(rec_vars)
    inh_cells[[0, 1]].record(rec_vars) # is recording two cells, for getting all the pop cells use  inh_cells.record(rec_vars)       
else:
...
filename = normalized_filename("scripts/VA/Results", "VAbenchmarks_%s_exc" % options.benchmark, "pkl",
